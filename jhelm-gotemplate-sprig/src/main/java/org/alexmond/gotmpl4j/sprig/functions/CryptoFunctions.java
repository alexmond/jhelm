...
public final class CryptoFunctions {
	...
	/** BCrypt cost, matching Spring Security's BCryptPasswordEncoder default strength. */
	 ...
	 * BouncyCastle so the engine carries no Spring dependency. BCrypt uses only the first
	 * 72 password bytes; longer inputs are truncated to keep BouncyCastle from rejecting
	 * them, mirroring the reference implementation.
	 */
	private static String bcryptHash(String password) {
		...
		if (pw.length > 72) {
			...
			System.arraycopy(pw, 0, truncated, 0, 72);
			pw = truncated;
		}
		return OpenBSDBCrypt.generate("2y", pw, salt, BCRYPT_COST);
	}
	...
	/**
	 * Generates an htpasswd entry using BCrypt hashing (BouncyCastle).
	 * @return htpasswd entry in format {@code "username:$2y$hash"}
	 */
	private static Function htpasswd() {
		return (args) -> {
			...
			return username + ":" + bcryptHash(password);
		};
	}
	...
	private static Function bcrypt() {
		return (args) -> {
			...
			return bcryptHash(String.valueOf(args[0]));
		};
	}
