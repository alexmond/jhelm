...
. Add function to the appropriate category class (e.g., `StringFunctions.java`)
. Function will be automatically registered via the coordinator (`HelmFunctions` or `SprigFunctionsRegistry`)
. Add tests in corresponding test class (e.g., `Helm4FunctionsTest.java`)
. Update `getFunctionCategories()` in the coordinator if adding a new category
