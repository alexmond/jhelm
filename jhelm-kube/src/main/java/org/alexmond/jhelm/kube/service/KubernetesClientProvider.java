...
public class KubernetesClientProvider implements KubernetesProvider {
	...
	private Object fetchCoreV1Resource(String kind, String namespace, String name) throws ApiException {
		return switch (kind) {
			case "Pod" ->
				name.isEmpty() ? coreV1Api.listNamespacedPod(namespace) : coreV1Api.readNamespacedPod(name, namespace);

			case "Service" -> name.isEmpty() ? coreV1Api.listNamespacedService(namespace)
					: coreV1Api.readNamespacedService(name, namespace);

			case "ConfigMap" -> name.isEmpty() ? coreV1Api.listNamespacedConfigMap(namespace)
					: coreV1Api.readNamespacedConfigMap(name, namespace);

			case "Secret" -> name.isEmpty() ? coreV1Api.listNamespacedSecret(namespace)
					: coreV1Api.readNamespacedSecret(name, namespace);

			case "Namespace" -> name.isEmpty() ? coreV1Api.listNamespace() : coreV1Api.readNamespace(name);

			case "PersistentVolumeClaim" -> name.isEmpty() ? coreV1Api.listNamespacedPersistentVolumeClaim(namespace)
					: coreV1Api.readNamespacedPersistentVolumeClaim(name, namespace);

			case "ServiceAccount" -> name.isEmpty() ? coreV1Api.listNamespacedServiceAccount(namespace)
	...
	private Object fetchAppsV1Resource(String kind, String namespace, String name) throws ApiException {
		return switch (kind) {
			case "Deployment" -> name.isEmpty() ? appsV1Api.listNamespacedDeployment(namespace)
					: appsV1Api.readNamespacedDeployment(name, namespace);

			case "StatefulSet" -> name.isEmpty() ? appsV1Api.listNamespacedStatefulSet(namespace)
					: appsV1Api.readNamespacedStatefulSet(name, namespace);

			case "DaemonSet" -> name.isEmpty() ? appsV1Api.listNamespacedDaemonSet(namespace)
					: appsV1Api.readNamespacedDaemonSet(name, namespace);

			case "ReplicaSet" -> name.isEmpty() ? appsV1Api.listNamespacedReplicaSet(namespace)
	...
	private Map<String, Object> createMetadataMap(V1ObjectMeta metadata) {
		...
		if (metadata.getNamespace() != null) {
			map.put("namespace", metadata.getNamespace());
		}
		if (metadata.getLabels() != null) {
			map.put("labels", metadata.getLabels());
		}
		if (metadata.getAnnotations() != null) {
			map.put("annotations", metadata.getAnnotations());
		}
		if (metadata.getUid() != null) {
			map.put("uid", metadata.getUid());
		}
		if (metadata.getResourceVersion() != null) {
			map.put("resourceVersion", metadata.getResourceVersion());
		}
